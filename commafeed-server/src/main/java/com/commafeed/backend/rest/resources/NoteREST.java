...
public class NoteREST {
    ...
    public NoteREST(AuthenticationContext authenticationContext,
    ...
    public Response createNote(@Valid NoteRequest req) {
        ...
        FeedEntryNote note = feedEntryNoteService.createOrAttach(user, req.getEntryId(), req.getText(), req.getRating());
        if (note == null) {
            return Response.status(Response.Status.NOT_FOUND).build();
        }
    ...
    @GET
    @Transactional
    public Response listNotes(@QueryParam("offset") @DefaultValue("0") int offset,
                              @QueryParam("limit") @DefaultValue("50") int limit) {
        User user = authenticationContext.getCurrentUser();
        if (user == null) {
            return Response.status(Response.Status.UNAUTHORIZED).build();
        }
        List<NoteResponse> notes = feedEntryNoteService.listUserNotes(user, offset, limit)
